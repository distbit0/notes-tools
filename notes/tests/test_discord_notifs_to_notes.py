...
def test_run_stops_before_auth_while_polling_is_disabled(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def fail_if_auth_is_loaded() -> str:
        pytest.fail("disabled Discord polling attempted to load auth")

    monkeypatch.setattr(
        discord_notifs_to_notes,
        "load_discord_auth_token",
        fail_if_auth_is_loaded,
    )

    assert discord_notifs_to_notes.run() == 0
