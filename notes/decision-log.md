...
- A conversation absent from Brave history qualifies unconditionally; a visited conversation qualifies only when its latest visible assistant message is more than ten minutes newer than its latest Brave visit.
...
- The first full scan appends qualifying URLs uniquely and oldest-first to a mode-0600 text file because launching the initial backlog crashed Brave. Later incremental scans open qualifying conversations directly in Brave; queue-only delivery was never intended to be permanent. Conversations that remain in the initial queue are excluded from automatic opening until the user removes their URLs after processing them.
