...
- Qualifying URLs are appended uniquely and oldest-first to a mode-0600 text file rather than opened directly because launching the initial backlog crashed Brave. The user removes processed URLs from that file; a later update can then queue the same conversation again.
